...
const StudentForm: React.FC = () => {
  ...
  const handleSubmit = () => {
    if (!form.id || !form.name) return alert("Vui lòng nhập đầy đủ thông tin!");
  ...
  return (
    <div className="w-1/3 p-4 border rounded-xl shadow">
      ...
      <div className="flex flex-col gap-4">
        <TextField label="Mã sinh viên" name="id" value={form.id} onChange={handleChange} fullWidth />
        <TextField label="Tên sinh viên" name="name" value={form.name} onChange={handleChange} fullWidth />
        ...
        <TextField type="date" label="Ngày sinh" name="birthday" value={form.birthday} onChange={handleChange} fullWidth InputLabelProps={{ shrink: true }} />
        <TextField label="Nơi sinh" name="hometown" value={form.hometown} onChange={handleChange} fullWidth />
